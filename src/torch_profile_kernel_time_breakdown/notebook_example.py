...
df_agg_by_category = pl.concat(
    [
        x["df_agg_by_category"].with_columns(pack_name=pl.lit(x["pack_name"]))
        for x in df_pack_arr
    ]
...
for stage in ["prefill", "decode"]:
    df = df_agg_by_category.filter(pl.col("pack_name").str.contains(stage)).drop(
        "count_kernel"
    )
    fig = px.bar(
        df.to_pandas(),
...
for title, output_name, interest_legend_and_pack_names in [
    ...
    (
        ...
        [
            ...
            (
                "SGLang + Simulated Slow Attention",
                "sglang_decode/simulated_slow_attn",
            ),
        ],
    ),
    ...
    (
        ...
        [
            ...
            (
                "No TBO + 256 tok/device",
                "sglang_decode_no_tbo/in1800_bs256",
            ),
            (
                "No TBO + 128 tok/device",
                "sglang_decode_no_tbo/in1800_bs128",
            ),
        ],
    ),
    ...
    df = df_agg_by_category.filter(pl.col("pack_name").is_in(interest_names)).drop(
        "count_kernel"
    )
    df = df.with_columns(
        sort_index=pl.col("pack_name").replace(
            {name: i for i, name in enumerate(interest_names)}
        )
    ).sort("sort_index")
    # display(df)
    ...
    for i, pack in enumerate(pack_names):
        ...
        category_data = {
            row["category"]: (row["dur_mean_scaled"], row["dur_std_scaled"])
        ...
        label = {pack_name: text for text, pack_name in interest_legend_and_pack_names}[
            pack
        ]
    ...
    ax.grid(
        axis="x", color="#c5c5c5", linestyle="-", linewidth=0.5, alpha=0.5, zorder=-100
    )
    ...
    d = Path(
        "/Volumes/MyExternal/ExternalRefCode/lm-sys.github.io-private/public/images/blog/large_scale_ep"
    )
