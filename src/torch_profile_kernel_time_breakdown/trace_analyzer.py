...
def analyze_trace_raw(
    ...
    df = df.filter((pl.col('ts') >= interest_time_range[0] / 1000) & (pl.col('ts') < interest_time_range[1] / 1000))
    # df = df.filter(reduce(
    #     lambda a, b: a | b,
    ...
    assert ((len(df.filter(pl.col('name') == per_scenario_info.layer_indicator_kernel)) + 1)
            % layer_indicator_kernel_sparsity == 0)

    df = df.with_columns(
        section_index=((pl.col('name') == per_scenario_info.layer_indicator_kernel).cum_sum() - 1)
                      // layer_indicator_kernel_sparsity,
    )
    df = df.with_columns(
        name_index_in_section=pl.col("name").cum_count().over(["section_index", "name"]) - 1,
    )
    df = df.with_columns(
    ...
    df = df.with_columns(
        category=pl.col('name').replace_strict(category_of_name, default='Other Kernels'),
    )
    ...
    df = df.agg(
        count_kernel=pl.len(),
        dur_mean=pl.col('dur').mean(), dur_std=pl.col('dur').std(),
    )
    ...
    df = df.with_columns(layer_dur=pl.col('ts').diff()).filter(pl.col('layer_dur').is_not_null())
    df = df['layer_dur']
    row_overall = dict(category='Overall', dur_mean=df.mean(), dur_std=df.std(), count_kernel=None)

    df = df_raw
    df = df.group_by('section_index', 'bubble_mode').agg(dur=pl.col('dur_bubble_before_kernel').sum())
    ...
    sorted_categories = ['Overall'] + list(per_scenario_info.category_info) + [
        'Other Kernels',
        'Comp Stream Bubble',
        'Comm Stream Bubble',
        'Launch Overhead',
    ...
    df = df \
        .with_columns(
        category_sort_index=pl.col('category').replace({name: i for i, name in enumerate(sorted_categories)}).cast(int)) \
...
def _compute_bubble(df_raw):
    ...
    for tid in tids:
        ...
        df = df.with_columns(
            dur_bubble_before_kernel=(pl.col('ts') - pl.col('prev_ts_end')).clip(0, 1000000000),
        )
        df = df.with_columns(
            bubble_mode=pl.when(pl.col('dur_bubble_before_kernel') >= 20).then(
                pl.lit(f'{stream_name} Bubble')
